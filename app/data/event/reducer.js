...
export let events = (state = defaultState, action) => {
  ...
  switch (action.type) {
    case types.EVENTS.ADD_USER_EVENTS_LIST_INFO:
      return assoc('userEvents', { list: payload.info, loaded: true });
    case types.EVENTS.ADD_NEW_EVENTS:
      return assoc('newEvents', { list: payload.info, loaded: true });
    case types.EVENTS.ADD_RECOMMENDED_EVENTS:
      return assoc('recommended', { list: payload.info, loaded: true });
    default:
      return state;
  }
};
